...
#特殊的系数矩阵，这个矩阵转得到的YUV，可以很方便地转换成RGB
#其拟矩阵为：
# Matrix3x3 fastYuvToRgb = {
#     {1.0,  0.0,  2.0},    // R = Y + (Cr << 1)
#     {1.0, -0.5, -1.0},    // G = Y - (Cb >> 1) - Cr  
#     {1.0,  2.0,  0.0}     // B = Y + (Cb << 1)
# };
#将这三个式子右边除了Y的数分别记为d_r, d_g, d_b，但是这三个值可能超过sint8范围
#而GBA里面的RGB是555格式，本身计算完成之后就要>>>3
#因此，可以把d_r, d_g, d_b的值都右移1位，这样就可以保证它们在sint8范围内，同时Y的值也右移1位
#在计算的时候，就可以直接Y+d_r, Y+d_g, Y+d_b来得到RGB值，再右移两位就得到RGB555的值了，这样对精度影响也不高
#之所以不直接右移3位，是因为实测发现，这样精度会有明显损失
